...
import os
```
导入 Python 的 `os` 模块，它提供了与操作系统交互的功能，例如文件路径操作。
...
sys.path.append(project_root)
```
导入 Python 的 `sys` 模块，它提供了对 Python 解释器相关变量和函数的访问。这里的主要目的是将项目的根目录添加到 Python 的模块搜索路径 `sys.path` 中。这样做是为了确保在运行脚本时，Python 能够找到项目中自定义的模块（例如 `datasets` 目录下的模块），避免出现 `ModuleNotFoundError`。`os.path.abspath` 和 `os.path.join` 用于构建项目根目录的绝对路径，确保路径的正确性和跨平台兼容性。
